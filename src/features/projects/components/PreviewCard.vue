...
const FEATURED_SLUGS = ["crypto-dashboard"];
...
const isFeatured = computed(() => !!props.preview && FEATURED_SLUGS.includes(props.preview.slug));
...
<template>
  ...
  >
    <div class="preview-card-top" ref="wrapperRef">
      ...
      <div v-if="isFeatured" class="preview-card-crown" :aria-label="t('featured')">
        <Crown class="preview-card-crown-icon" />
      </div>
  ...
  >
    <div class="preview-card-top preview-card-top-empty">
      <Download class="preview-card-top-empty-icon" />
    </div>
    <div class="preview-card-content">
      <div class="preview-card-copys">
        <h3 class="preview-card-title">{{ t("download-cv") }}</h3>
        <p class="preview-card-description">{{ t("download-cv-description") }}</p>
      </div>
    </div>
  </a>
</template>
...
.preview-card {
  ...
  &-crown {
    ...
    filter: drop-shadow(0 6px 12px rgba(245, 197, 66, 0.55)) drop-shadow(0 4px 8px rgba(0, 0, 0, 0.45));
    pointer-events: none;
