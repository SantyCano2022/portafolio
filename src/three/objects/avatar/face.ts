...
const getMaterial = (): Material | null => {
  ...
  material = new ShaderMaterial({
    ...
    uniforms: {
      ...
      uFeatureTint: { value: new Color(0.05, 0.05, 0.05) },
      uFeatureStrength: { value: 1.0 },
      ...uniforms,
      ...avatar.uniforms,
    },
  });
...
const tick = () => {
  ...
  if (isContact) {
    ...
    if (isAbout) {
      uniforms.uFrame.value = FRAME_INDEXES[`proud-${Math.round(blinkFrame.value)}` as keyof typeof FRAME_INDEXES];
    } else {
