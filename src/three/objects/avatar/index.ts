...
// ───────────────────────────────────────────────────────────
// Avatar tinting per material (Santiago palette)
// Skin: warm latino/mediterranean tone
// Black: deep brown-black hair
// Gray: muted earth-tone clothing
// White: warm off-white
// ───────────────────────────────────────────────────────────
const TINTS: Record<string, { color: [number, number, number]; strength: number }> = {
  skin: { color: [1.35, 1.05, 0.85], strength: 1.0 },
  black: { color: [0.05, 0.05, 0.05], strength: 1.0 },
  gray: { color: [1.0, 1.0, 1.0], strength: 0.0 },
  white: { color: [1.0, 1.0, 1.0], strength: 0.0 },
};
...
const getMaterial = (name: string): Material | null => {
  ...
  if (name === "head") {
    ...
    return new ShaderMaterial({
      ...
      uniforms: {
        ...
        uHairTint: { value: new Color(0.05, 0.05, 0.05) },
        uHairStrength: { value: 1.0 },
        ...uniforms,
      },
    });
  }
  ...
  const tintData = TINTS[name];
  const tintColor = tintData ? new Color(tintData.color[0], tintData.color[1], tintData.color[2]) : new Color(DEFAULT_TINT[0], DEFAULT_TINT[1], DEFAULT_TINT[2]);
  const tintStrength = tintData ? tintData.strength : 0;
